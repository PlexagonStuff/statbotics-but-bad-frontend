...
export function Dropdown ({searchText}) {
    ...
    const teamsList = filteredTeams.map((team,index) => (
        ...
        <TeamLink key={index} teamInfo={team}/>));
    return (<div>
        <div className="grid mt-10 grid-rows-10 grid-cols-1 overflow-x-clip overflow-y-clip w-48 max-h-44 bg-zinc-700 rounded-md">
             {teamsList}
        </div>
    </div>)
}